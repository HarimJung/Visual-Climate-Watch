...
test('A country with no parsed target reports no target, not a borrowed one',()=>{const d=load('KEN');assert.notEqual(d.ndc_document?.state,'pledged','the specimen must be a country whose filing yielded no figure');assert.equal(d.ndc.target_emissions_mtco2e,null);assert.equal(d.ndc.reduction_pct,null);assert.equal(d.derived.on_track,null);});
