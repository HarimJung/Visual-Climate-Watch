...
export default function MovementScene({data,controls,onSelect,onReady,onPhase,onProgress}:{data:CountryData;controls:SceneControls;onSelect:(id:string)=>void;onReady?:(canvas:HTMLCanvasElement)=>void;onPhase?:(index:number)=>void;onProgress?:(value:number)=>void}){
 ...
 // One rule for the whole machine: a part with no figure behind it is not drawn
 // full. The target ring falls to zero, an unread seat stays an empty bore, the
 // BTR bridge never seats, and the inventory train turns with the sign of the
 // measured trend, standing still when no trend could be measured at all.
 const trend=data.derived.trend_annual_mtco2e??null;
 // Falling emissions wind the train forward; rising emissions reverse it. The
 // meshing signs stay alternating either way, so the train stays mechanical.
 const turn=trend==null?0:trend<0?1:-1;
