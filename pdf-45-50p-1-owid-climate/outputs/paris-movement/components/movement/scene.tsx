...
export default function MovementScene({data,controls,onSelect,onReady,onPhase,onProgress}:{data:CountryData;controls:SceneControls;onSelect:(id:string)=>void;onReady?:(canvas:HTMLCanvasElement)=>void;onPhase?:(index:number)=>void;onProgress?:(value:number)=>void}){
 ...
 setFailed(false);const el=host.current;const scene=new T.Scene();const renderer=new T.WebGLRenderer({antialias:true,alpha:true,preserveDrawingBuffer:true});renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));renderer.setClearColor(0xf1f2f1,0);renderer.shadowMap.enabled=true;renderer.shadowMap.type=T.PCFSoftShadowMap;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=.92;el.appendChild(renderer.domElement);renderer.outputColorSpace=T.SRGBColorSpace;renderer.domElement.setAttribute('aria-label','기후 데이터 조립 모형. 드래그로 회전, 휠로 확대, 탭으로 부품 선택.');ready.current?.(renderer.domElement);
 ...
 const anchorN=new T.Object3D();anchorN.position.set(-1.7,2.4,.37);promise.add(anchorN);label(anchorN,'01 / NDC 목표 링',`${fmt(data.ndc.reduction_pct)}% 감축 약속`,'#356aef',1,[-110,-28]);
 ...
 sourceGears.forEach(([x,y,r,n,mat,sign,id])=>{const g=clickable(cog(inventory,x,y,r,n,mat,sign),id);g.userData.source=id;const available=id==='delivery'?data.series.observed.length>0:(data.sources??[]).some(s=>s.id===id.slice(7)&&s.connection==='connected');g.userData.available=available;if(!available){g.traverse(o=>{if(o instanceof T.Mesh){const original=o.material as THREE.MeshStandardMaterial;const m=original.clone();m.transparent=true;m.opacity=.23;m.depthWrite=false;o.material=m;}});}const cap=new T.Group();cap.position.set(x,y,.43);inventory.add(cap);engraving(cap,id==='delivery'?'INVENTORY':id==='source:DS-05'?'EDGAR':'TRACE','#314c61',r*1.22,r*.30,.02);clickable(cap,id);});
 ...
 const anchorI=new T.Object3D();anchorI.position.set(-2.1,-1.65,.3);inventory.add(anchorI);label(anchorI,'02 / 관측 데이터',`${data.series.observed.length}개 관측 연도`,'#167f7c',2,[-144,10]);
 ...
 const anchorB=new T.Object3D();anchorB.position.set(1.8,1.3,.27);evidence.add(anchorB);label(anchorB,'03 / BTR 보고 의무',data.btr.submitted===true?`제출됨 · ${Object.values(data.btr.components).filter(c=>c.state==='unknown').length}개 미파싱`:'제출 기록 미파싱','#8060c4',3,[36,-42]);
 // 4. Finance docks beside the mechanism. Unknown receipts never make this conditional wheel mesh.
 const finance=part('conditions',4,.44);finance.position.set(3.58,-.38,.44);const fg=cog(finance,0,0,.55,23,gold,-1);gears.pop();clickable(fg,'conditions');ring(finance,.68,.018,.13,edge);cylinder(finance,.11,.15,.15,gold);label(finance,'04 / 국제 지원 조건',data.finance_need.received_usd==null?'받은 재원 미확인 → 미결합':'재원 수령과 조건 충족은 별개','#ab7728',4,[32,18]);
 ...
 function updateLabels(){
  ...
  labels.forEach(({el:tag,anchor,offset,phase},i)=>{
   ...
   const show=amount<=.96&&inDepth&&cx>=0&&cx<=el.clientWidth&&cy>=0&&cy<=el.clientHeight&&(el.clientWidth>=600||i<2);
