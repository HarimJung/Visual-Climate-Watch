...
import {validateCountry,type CountryData} from '@/lib/climate';
...
export async function loadView<T>(name:'refusals'|'divergence'):Promise<T|null>{
 try{
  const r=await fetch(new URL(`/data/${name}.json`,await origin()),{signal:AbortSignal.timeout(10000),headers:{Accept:'application/json'}});
...
export async function loadCountry(iso3:string,origin:string):Promise<CountryData|null>{
 ...
 const url=upstream?upstreamUrl(upstream,iso3):new URL(`/data/countries/${iso3}.json`,origin);
 let r:Response;
 try{r=await fetch(url,{signal:AbortSignal.timeout(10000),headers:{Accept:'application/json'}})}
