...
- **R10은 판정에도 적용된다 (2026-09-12).** 문서가 쓴 톤수(인용 목표, 문서 BAU×퍼센트)는
  문서의 인벤토리 위에 있고 추세는 다른 소스 위에 있다. 두 장부면 판정하지 않고
  거절한다(`gap.two-ledgers`). 판정은 퍼센트를 추세 소스 자신의 기준연도 값에
  적용한 경우뿐이고, 그때 `derived.trend_source_id`와 판정문이 장부를 말한다.
