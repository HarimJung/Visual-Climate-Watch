...
### M10 — BAU 투영 파싱 (기능1의 병목)
`ds-06-ndc-docs.ts`에 BAU 총량 매처 추가. 읽으면 `ndc.bau_2030_mtco2e` +
`bau_state:'pledged'`가 채워지고 `derive()`가 BAU형 목표를 판정할 수 있다.
§4.3의 함정 전부에 대해 회귀 테스트를 먼저 쓴다 (R10).

**인수:** 판정 국가 4 → 두 자리. 오판정 0. `ndc-extract.test.ts` 전부 통과.
